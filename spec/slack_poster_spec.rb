...
describe 'slack_poster' do
  ...
  let(:fake_slack_poster_class) { instance_double(Slack::Poster) }

  context 'send_request' do
    before do
      expect(Slack::Poster).to receive(:new).and_return(fake_slack_poster_class)
      Timecop.freeze(Time.local(2015, 07, 16))
    end

    it 'posts to Slack' do
      expect(fake_slack_poster_class).to receive(:send_message).with(message).and_return(:ok)
