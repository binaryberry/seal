...
class GithubFetcher
	...
	TEAM_MEMBERS_ACCOUNTS = ["binaryberry", "jamiecobbett", "boffbowsh", "alicebartlett", "benilovj", "fofr", "russellthorn", "tekin"]
	...
	def list_pull_requests
		@repos.each do |repo|
			...
			response.body.each{|pull_request| @pull_requests << pull_request.title if pull_request_valid?(pull_request)}
		end
		@pull_requests
	end
